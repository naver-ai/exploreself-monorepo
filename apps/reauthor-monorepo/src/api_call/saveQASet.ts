...
export const updateResponse = async (token: string, qid: string, response: string) => {
  try {
    const resp = await Http.axios.put(`/response/${qid}}`, {
      response: response,
    },{
      headers: Http.makeSignedInHeader(token)
    })
...
export const saveComment = async(token: string, qid: string, comment: string) => {
  try {
    const response = await Http.axios.put(`/response/comment/${qid}`,{
      comment: comment
    },{
      headers: Http.makeSignedInHeader(token)
    })
